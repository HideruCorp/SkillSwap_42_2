...
const normalizeForSearch = (text: string): string[] => {
  ...
  return [
    lowerText,
    transliterateToEnglish(lowerText),
    transliterateToRussian(lowerText)
  ].filter((variant, index, array) =>
    variant && array.indexOf(variant) === index
  );
};
...
export default function filterUsers(
  ...
  return users.filter((user) => {
    ...
    if (filters.textSearch && filters.textSearch.trim() !== '') {
      ...
      const matchesName = matchesWithTransliteration(userName, searchTerm) ||
