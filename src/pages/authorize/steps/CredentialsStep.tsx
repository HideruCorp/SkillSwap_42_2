...
} from '@widgets/forms/credentials-form';
...
function CredentialsStep({ onLoginSuccess }: CredentialsStepContainerProps) {
  ...
  const externalErrors: CredentialsFormErrors | undefined = useMemo(() => {
    ...
    if (stepErrors?.email) errors.email = stepErrors.email;
    if (stepErrors?.password) errors.password = stepErrors.password;

    if (loginError) errors.root = loginError;
